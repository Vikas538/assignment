...
router.post("/analytics/visited", auth_token, async (req, res) => {
  ...
  if (method) {
    ...
    while (from_month <= to_month) {
      ...
      let query = `SELECT count (*) AS total FROM tickets WHERE movie_title = '${movie_name}' and EXTRACT(MONTH FROM creation_date)=${from_month}`;
